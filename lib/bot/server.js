...
async function createPaymentLink(orderId, amount, userId, fileName) {
  try {
    ...
    // Base64 encode server key untuk authentication
    const authString = Buffer.from(serverKey + ":").toString("base64");

    const response = await fetch(
      isProduction
        ? "https://api.midtrans.com/v2/payment-links"
        : "https://api.sandbox.midtrans.com/v2/payment-links",
      {
        method: "POST",
        headers: {
    ...
    if (data.status_code !== "201") {
      throw new Error(data.status_message || "Failed to create payment link");
    }
...
async function handlePayCommand(ctx) {
  return new Promise((resolve) => {
    ...
    ctx
      ...
      .then(() => generatePaymentLink(ctx, userSession.cost))
      .then(() => {
...
async function generateQRIS(orderId, amount) {
  try {
    ...
    // Coba menggunakan Core API terlebih dahulu
    const coreApiResult = await generateQRISCoreAPI(orderId, amount);
    if (coreApiResult.success) {
      return coreApiResult;
    }

    console.log("🔄 Falling back to Snap API");
    ...
    if (isProduction && transaction.qr_code) {
      ...
      // Generate QR code dari redirect_url
...
// Fungsi untuk generate QRIS menggunakan Core API
async function generateQRISCoreAPI(orderId, amount) {
  try {
    const isProduction = process.env.MIDTRANS_ENVIRONMENT === "production";
    const serverKey = process.env.MIDTRANS_SERVER_KEY;
    ...
    const response = await fetch(
        ...
        : "https://api.sandbox.midtrans.com/v2/charge",
      {
        ...
        body: JSON.stringify({
          ...
          qris: {
            acquirer: "gopay", // atau 'dana', 'linkaja'
          },
        }),
      }
    );
    ...
    if (data.status_code !== "201") {
      throw new Error(
        data.status_message || "Failed to create QRIS transaction"
      );
    }

    return {
      success: true,
      qr_code: data.actions.find((action) => action.name === "generate-qr-code")
        ?.url,
      payment_url: data.actions.find((action) => action.name === "qr-code")
        ?.url,
      transaction_id: data.transaction_id,
      order_id: data.order_id,
    };
  } catch (error) {
    console.error("❌ Core API QRIS generation error:", error);
    return {
      success: false,
      error: error.message,
    };
...
// Ganti fungsi simulateQRISGeneration dengan ini
async function generatePaymentLink(ctx, amount) {
  try {
    ...
    console.log("💰 Generating Payment Link for amount:", amount);

    // Generate Payment Link menggunakan Midtrans
    ...
    userSession.paymentInfo = {
      orderId: orderId,
      amount: amount,
      transactionId: paymentLinkResult.transaction_id,
    ...
    console.log("🔗 Payment URL:", paymentLinkResult.payment_url);
    ...
    await ctx.reply(
      `💳 *Payment Link Created*\n\n` +
        ...
        `Pembayaran akan diverifikasi otomatis. Ketik /status untuk mengecek status pembayaran.`,
      { parse_mode: "Markdown" }
    );
    ...
    let errorMessage = "❌ Gagal membuat payment link. ";
...
// Perbarui fungsi pengecekan status pembayaran
function startPaymentStatusCheck(ctx, orderId) {
  const userId = ctx.from.id;
  let checks = 0;
  const maxChecks = 30; // 5 menit (30 * 10 detik)
  ...
  const interval = setInterval(async () => {
    ...
    if (status && status.transaction_status !== "pending") {
      ...
      const userSession = userSessions.get(userId);
      if (userSession && userSession.paymentInfo) {
        userSession.paymentInfo.status = status.transaction_status;
        userSessions.set(userId, userSession);
        ...
        await sendPaymentStatusUpdate(
          ...
          orderId,
          userSession.paymentInfo.amount
        );
...
async function processSuccessfulPayment(userId, userSession) {
  try {
    ...
    // Proses printing
    await processPrinting(userId, userSession);
  } catch (error) {
    console.error("Error processing successful payment:", error);
  }
}

// Fungsi untuk menangani pembayaran expired
async function processExpiredPayment(userId, userSession) {
  try {
    // Reset payment info, biarkan user mencoba lagi
    userSession.paymentInfo = null;
    userSession.currentStep = "settings_configured";
    userSessions.set(userId, userSession);
...
async function processFailedPayment(userId, userSession) {
  try {
    // Reset payment info, biarkan user mencoba lagi
    userSession.paymentInfo = null;
    userSession.currentStep = "settings_configured";
    ...
    await global.botInstance.telegram.sendMessage(
      userId,
      "❌ Pembayaran gagal. Silakan coba lagi dengan /pay atau gunakan metode pembayaran lain."
    );
  } catch (error) {
...
// Di bagian akhir file, sebelum module.exports, tambahkan:
